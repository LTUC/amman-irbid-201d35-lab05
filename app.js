...
function multiply(a, b) {
  var result = a * b;
  var sent = `The product of ${a} and ${b} is ${result}.`;
  var resultArray = [result, sent];
  return resultArray;
...
function sumArray(sumArr) {
  ...
  var first = sum(sumArr[0], sumArr[1])[0];
  totalSum = sum(first, sumArr[2])[0];
...
function multiplyArray(multArr) {
  var Array = multArr.join();
  var sent = '';
  var firstm = multiply(multArr[0], multArr[1])[0];
  var totalm = multiply(firstm, multArr[2])[0];

  sent = `The numbers ${Array } have a product of ${totalm}.`;
  return [totalm, sent];
}
